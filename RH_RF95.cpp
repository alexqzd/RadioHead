...
void RH_RF95::validateRxBuf()
{
    ...
    _rxHeaderTo = (_buf[0] << 24) | (_buf[1] << 16) | (_buf[2] << 8) | (_buf[3]);
    _rxHeaderFrom = (_buf[4] << 24) | (_buf[5] << 16) | (_buf[6] << 8) | (_buf[7]);
    _rxHeaderId = _buf[8];
    _rxHeaderFlags = _buf[9];
    if (_promiscuous ||
...
bool RH_RF95::send(const uint8_t *data, uint8_t len)
{
    ...
    // First the 4 headers
    spiWrite(RH_RF95_REG_00_FIFO, (_txHeaderTo >> 24) & 0xFF);
    spiWrite(RH_RF95_REG_00_FIFO, (_txHeaderTo >> 16) & 0xFF);
    spiWrite(RH_RF95_REG_00_FIFO, (_txHeaderTo >> 8) & 0xFF);
    spiWrite(RH_RF95_REG_00_FIFO, (_txHeaderTo >> 0) & 0xFF);
    // spiWrite(RH_RF95_REG_00_FIFO, _txHeaderTo);

    spiWrite(RH_RF95_REG_00_FIFO, (_txHeaderFrom >> 24) & 0xFF);
    spiWrite(RH_RF95_REG_00_FIFO, (_txHeaderFrom >> 16) & 0xFF);
    spiWrite(RH_RF95_REG_00_FIFO, (_txHeaderFrom >> 8) & 0xFF);
    spiWrite(RH_RF95_REG_00_FIFO, (_txHeaderFrom >> 0) & 0xFF);
