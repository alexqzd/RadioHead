...
public:
    ...
    RHReliableDatagram(RHGenericDriver& driver, uint32_t thisAddress = 0);
    ...
    bool sendtoWait(uint8_t* buf, uint8_t len, uint32_t address);
    ...
    bool recvfromAck(uint8_t* buf, uint16_t* len, uint32_t* from = NULL, uint32_t* to = NULL, uint8_t* id = NULL, uint8_t* flags = NULL);
    ...
    bool recvfromAckTimeout(uint8_t* buf, uint16_t* len,  uint16_t timeout, uint32_t* from = NULL, uint32_t* to = NULL, uint8_t* id = NULL, uint8_t* flags = NULL);
...
protected:
    ...
    void acknowledge(uint8_t id, uint32_t from);
...
private:
    ...
    /// Map of the last seen sequence number indexed by node address that sent it
    ...
    std::map<uint32_t, uint8_t> _seenIdsforAddress;
};
